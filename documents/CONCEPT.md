...
- CharacterEditor: flowing page with three block types — text (plain prose, no card), field (bordered horizontal row), note (bordered titled card). Toolbar: + Field, Fields (rapid-entry toggle), + Note, ⚄ Random. InsertZone taps between/after structured blocks create text blocks. Trailing InsertZone always visible (64px). TextBlock/FieldBlock/NoteBlock delete confirmation all inline right. Sort toggle (Document / Type / A–Z). Color picker. Auto-saves 600ms debounced. New characters seed with empty text block + Role/Age/Motivation fields. No browser focus ring on web. Field rapid-entry mode: Enter in label → value, Enter in value → next field.
