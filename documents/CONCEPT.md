...
- CharacterEditor: holistic text model — TextBlocks have no delete button (select+backspace to delete). Deleting a field/note between two text blocks auto-merges them. Trailing InsertZone appends at true end. New character creation auto-focuses name input (shown as placeholder, not pre-filled); Enter/Tab advances to first block. Toolbar: + Field, Fields (rapid-entry), + Note, → Note/→ Text (convert), ⚄, sort (Document/Type/A–Z). Convert button: with text selection → converts selection to note; no selection → converts whole block; on note → converts back to prose (title dropped). NoteBlock title: always-mounted TextInput, collapses to height:0 + tap zone when empty+unfocused; auto-focused on note creation/conversion. FieldBlock label width is dynamic (text mirror, min 60, max 200). InsertZone between blocks is 32px to match TextBlock height. Autocomplete: arrow keys navigate suggestions, Enter selects. No focus ring/outline/box-shadow on web.
