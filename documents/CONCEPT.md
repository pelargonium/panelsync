...
- CharacterEditor: holistic text model — TextBlocks have no delete button (select+backspace to delete). Deleting a field/note between two text blocks auto-merges them. Trailing InsertZone appends at true end. New character creation auto-focuses name input; Enter/Tab advances to first block. Toolbar: + Field, Fields (rapid-entry), + Note, → Note/→ Text (convert), ⚄, sort (Document/Type/A–Z). Convert button: with text selection → converts selection to note (surrounding text stays); no selection → converts whole block; on note → converts back to prose. Autocomplete: arrow keys navigate suggestions, Enter selects. No focus ring/outline/box-shadow on web. All focus styles suppressed via noOutline.
